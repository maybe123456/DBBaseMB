...
//        CommonUtils.sendMessageOnBackground(mContext, phoneNumber, msg);
        ...
        ZClient.getService(ZService.class).sendSmsCode(phoneNumber).enqueue(new ZCallback<ZResponse>() {
            @Override
...
//        }
        ...
        ZClient.getService(ZService.class).registerCode(phoneNumber, password, userName,verifyCode).enqueue(new ZCallback<ZResponse<UserBean>>() {
            @Override
