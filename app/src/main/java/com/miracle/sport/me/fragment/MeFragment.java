...
public class MeFragment extends BaseFragment<F4Ddz2Binding> {
    ...
    private void reqData() {
        ...
        ZClient.getService(SportService.class).getMyPostList(1, 10).enqueue(new ZCallback<ZResponse<List<PostBean>>>(binding.swipeLayout) {
            @Override
        ...
        ZClient.getService(SportService.class).getMyCircleList().enqueue(new ZCallback<ZResponse<List<MyCircleBean>>>(binding.swipeLayout) {
            @Override
        ...
        ZClient.getService(SportService.class).getMycollections(1, 10).enqueue(new ZCallback<ZResponse<List<Football>>>(binding.swipeLayout) {
            @Override
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.llMe:
                if (CommonUtils.getUser() == null || null == userInfo) {
